...
const TerminalPromptLine = ({ children }: { children?: React.ReactNode }) => (
    ...
    (cmd: string) => {
      ...
      if (trimmedCmd === "help") {
        setHistory((prev) => [
          ...prev,
          { command: cmd },
          { command: "", output: TERMINAL_HELP_TEXT },
        ]);
        return;
      }

      // Find matching command
      const matchedCommand = Object.keys(TERMINAL_COMMANDS).find(
        (key) => key.toLowerCase() === trimmedCmd
      );

      if (matchedCommand && TERMINAL_COMMANDS[matchedCommand].route) {
        setHistory((prev) => [
          ...prev,
          { command: cmd },
          {
            command: "",
            output: `navigating to ${TERMINAL_COMMANDS[matchedCommand].description}...`,
          },
        ]);
        // Small delay for visual feedback
        setTimeout(() => {
          router.push(TERMINAL_COMMANDS[matchedCommand].route);
        }, ANIMATION.terminal.navigationDelay);
      } else {
        setHistory((prev) => [
          ...prev,
          { command: cmd },
